...
socket.on('userConversations', (rows = []) => {
  ...
  rows.forEach((r) => {
    ...
    card.innerHTML = `<div style='display:flex;align-items:center;gap:.6rem;'><img class='pfp' src='${r.pfp || pfp}'><div><strong>${r.displayName}</strong><div class='ticket-meta'>@${r.username}</div><div class='ticket-meta'>${r.lastText || ''}</div></div></div><button class='btn open-btn'>Open</button>`;
