(function(){
  ...
  function upsertConversation(row){
    ...
    const base = {
      ...
      initials: (row.displayName || row.username).slice(0,2).toUpperCase(),
      status: 'online',
      role: 'MEMBER',
      kills: '0',
      points: '0',
      preview: row.lastText || '',
      time: 'now',
      unread: 0,
      messages: []
    };
  ...
  function pushAdminMessage(m){
    const next = {
      id: m.createdAt || Date.now(),
      from: m.fromAdmin ? 'admin' : (m.username === username ? 'me' : 'admin'),
      text: m.text || '',
      ts: fmt(m.createdAt),
      date: dlabel(m.createdAt)
    };
    ADMIN.messages.push(next);
    if (m.ticketId) activeTicketId = m.ticketId;
    document.getElementById('adminPreview').textContent = next.text || 'New message';
  }

  function loadTicketThreadForUser(){
    socket.emit('getTickets');
  }

  const oldOpen = window.openDm;
  window.openDm = function(id){
    activePeer = id === 'admin' ? null : id;
    oldOpen(id);
    if (id === 'admin') {
      loadTicketThreadForUser();
      return;
    }
    if (activePeer) socket.emit('getDirectMessages', { a: username, b: activePeer });
  };
  ...
  socket.on('ticketsUpdated', (rows = []) => {
    const mine = rows.find((t) => t.username === username && t.status === 'open');
    if (!mine) return;
    activeTicketId = mine.id;
    socket.emit('getTicketThread', mine.id);
  });

  socket.on('ticketThread', (ticket) => {
    if (!ticket || ticket.username !== username) return;
    activeTicketId = ticket.id;
    ADMIN.messages = (ticket.messages || []).map((m, i) => ({
      id: m.createdAt || i + 1,
      from: m.fromAdmin ? 'admin' : 'me',
      text: m.text || '',
      ts: fmt(m.createdAt),
      date: dlabel(m.createdAt)
    }));
  ...
  socket.on('userConversations', (rows = []) => {
    rows.forEach(upsertConversation);
    buildList(document.getElementById('searchInput')?.value || '');
  });

  socket.on('directMessages', (msgs = []) => {
    if (!activePeer) return;
    const conv = DMS.find((x) => x.id === activePeer);
    if (!conv) return;
    conv.messages = msgs.map((m, i) => ({
      id: i + 1,
      from: m.from === username ? 'me' : m.from,
      text: m.text,
      ts: fmt(m.createdAt),
      date: dlabel(m.createdAt)
    }));
