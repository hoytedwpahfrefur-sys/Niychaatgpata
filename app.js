...
app.get('/Server.html', (_req, res) => res.sendFile(path.join(__dirname, 'Server.html')));
app.get('/chat.html', (_req, res) => res.sendFile(path.join(__dirname, 'chat.html')));
...
app.post('/api/applications', (req, res) => {
  const { username, kills, skills, mainCharacter, submittedAt } = req.body || {};
  if (!username) return res.status(400).json({ error: 'username required' });
  applications.set(username, { username, kills: Number(kills || 0), skills: skills || '', mainCharacter: mainCharacter || '', submittedAt: submittedAt || Date.now(), status: 'pending' });
...
app.get('/72&#72--6.html', (_req, res) => res.sendFile(path.join(__dirname, 'hahahaiamafmjnyourbad6677.html')));
