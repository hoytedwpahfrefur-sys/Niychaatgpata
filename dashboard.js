...
document.getElementById('chatOpen').onclick = () => {
  dashChat.classList.remove('hidden');
  document.getElementById('sideMenu').classList.add('hidden-left');
};
...
document.getElementById('dashChatForm').addEventListener('submit', (e) => {
  ...
  if (!text) return;
  socket.emit('ticketMessage', { displayName: profile.displayName || username, pfp: profile.pfp || defaultPfp, text, username });
  input.value = '';
});
